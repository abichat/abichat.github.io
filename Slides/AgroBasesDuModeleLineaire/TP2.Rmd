...
```{r}
mat_cor <- cor(phytoNum)
mat_cor
mat_pcor <- pcor(phytoNum)
mat_pcor$estimate
```
...
* `AUD` apporte de l'information de manière significative en plus de `MOB` et `HIP` sur `BRW`,
* `MOB` apporte de l'information de manière significative en plus de `AUD` et `HIP` sur `BRW`,
* `HIP` apporte de l'information de manière significative en plus de `AUD` et `MOB` sur `BRW`.
