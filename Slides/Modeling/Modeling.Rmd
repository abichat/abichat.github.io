...
df_abund_transp %>% 
  ...
  select(Sample, .fittedPC1, .fittedPC2, .fittedPC3, .fittedPC4)
```
...
ggplot(df_clust) +
  geom_point(aes(x = .fittedPC1, y = .fittedPC2, color = .cluster, shape = Body_site)) +
  facet_grid(. ~ K) +
...
Using the `janitor::tabyl()` function, create a contingency table by K
...
df_phylum_transp_stool %>% 
  ...
  mutate(splits = map(splits, as_tibble), 
         Firmicutes = map(splits, pull, Firmicutes),
         Bacteroidetes = map(splits, pull, Bacteroidetes),
         Cor = map2_dbl(Firmicutes, Bacteroidetes, cor)) -> df_cor
df_cor
```
...
```{r fig.height=5}
ggplot(df_cor) +
  geom_density(aes(x = Cor), fill = "grey") +
  geom_vline(xintercept = 0, color = "grey") +
  geom_vline(xintercept = cor(df_phylum_transp_stool$Firmicutes, 
                              df_phylum_transp_stool$Bacteroidetes), 
             color = "tomato") 
```
