---
title: "<br>Take care of your data"
...
```{r nopipe}
2^mean(log(seq(10), base = 2), na.rm = TRUE)
```
...
The keybord shorcut is ctrl/⌘ + ⇧ + M


---
# Reaffectation pipe  `%<>%`

Usefull for updating an object
...
Usefull for side-effects, like plotting or printing
...
There are three interrelated rules which make the dataset tidy:
...
```{r separate}
grades %<>% separate(Sexage, into = c("Sex", "Age"))
...
```{r gather}
grades %<>% gather(Test1, Test2, Test3, key = Test, value = Grade)
knitr::kable(head(grades, 9), "html")
```
...
```{r}
library(tibble)
swiss %>% as.tibble(rownames = "Province")
```

--

.full-width[.content-box-sotr[Row names on a tibble are deprecated. They must be considered as variables.]]
...
data_syracuse <- 100 %>% 
  seq(from = 1, to = .) %>% 
  tibble(N = .) %>% 
  mutate(Sequence = Vectorize(syracuse)(N))
```
--
```{r show_syr_seq}
data_syracuse
...
```{r data_syracuse2}
data_syracuse %<>% 
  mutate(Length = map_dbl(Sequence, length),
...
```{r plotssyrac, fig.width=11, fig.height=6, fig.align='center'}
p1 <- plot_syrac(data_syracuse, Length)
p2 <- plot_syrac(data_syracuse, log10(Max)) + 
  ylab("log(Max)")
